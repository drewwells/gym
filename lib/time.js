...
// The UTC Date for a wall-clock time (hh:mm) on `dateStr` (YYYY-MM-DD) in `tz`.
// Unlike providers that return absolute ISO instants, LA Fitness gives only a
// local weekday + clock time, so we synthesize the instant here. Two-pass
// offset resolution keeps it correct across DST boundaries.
function tzWallClockUTC(dateStr, hh, mm, tz = DEFAULT_TZ) {
  const [y, mo, d] = dateStr.split('-').map(Number);
  const guess = Date.UTC(y, mo - 1, d, hh, mm, 0);
  let offsetMin = tzOffsetMinutes(new Date(guess), tz);
  offsetMin = tzOffsetMinutes(new Date(guess - offsetMin * 60000), tz);
